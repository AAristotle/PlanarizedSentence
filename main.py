    ...
    def eval(self, epoch, data_loader, is_test=False):
        ...
        with torch.no_grad():
            for i, data_batch in enumerate(data_loader):
                ...
                ent_c, ent_p, ent_r = utils.decode_2(outputs.cpu().numpy(), entity_text, length.cpu().numpy())
                # print("api_decode")
                ent_c_api, ent_p_api, ent_r_api = utils.api_decode(outputs.cpu().numpy(), entity_text, length.cpu().numpy())

                pred_entity = utils.write_outputs(outputs.cpu().numpy(), entity_text, length.cpu().numpy())

                total_ent_r += ent_r
                total_ent_p += ent_p
                total_ent_c += ent_c

                total_ent_r_api += ent_r_api
                total_ent_p_api += ent_p_api
                total_ent_c_api += ent_c_api
        ...
        e_f1_api, e_p_api, e_r_api = utils.cal_f1(total_ent_c_api, total_ent_p_api, total_ent_r_api)
        ...
        table.add_row(["Entity"] + ["{:3.4f}".format(x) for x in [e_f1_api, e_p_api, e_r_api]])
...
if __name__ == '__main__':
    ...
    logger.info("Trainer load model finished.....")
    ...
    hours = time_sum // 3600
    minutes = (time_sum - 3600 * (time_sum//3600))//60
    secs = round(time_sum - hours*3600 - minutes*60, 0)
    print('训练时间持续：', hours, '小时， ', minutes, '分钟， ', secs, '秒')
